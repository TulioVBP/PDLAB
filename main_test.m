...
damage.S0 = [-0.98 0.95*damage.Sc]; % S0- and S0+
damage.S1 = [-0.99 1.05*damage.Sc]; % S1- and S1+
% ---- Simulation
sigma = 4; % [MPa]
...
model.name = "Lipton Free Damage"; % "PMB", "Linearized LPS bond-based", "Lipton Free Damage" "LPS 2D" "Linearized LPS"
...
for s_index = 1:length(sigma)
    ...
    for m_index = 1:1%length(m_vec)
        ...
        %% -------------- Boundary conditions ----------------
        [ndof,idb,bc_set,bodyForce,noFailZone] = boundaryCondition(x,stresses,m,h,A,false,'btlr');
        %% -------------- GENERATE FAMILY ------------------
        ...
        switch solver
            case "Dynamic/Explicit"
                ...
                n_tot = length(t);
                [u_n,phi,energy] = solver_DynamicExplicit(x,t,idb,bodyForce,bc_set,family,partialAreas,T,c,rho,model,par_omega,history,noFailZone,damage);
            case "Quasi-Static"
                if damage.damageOn
                    error('Disable damage to run a quasi-static solver')
                end
                n_tot = 2;
                [u_n,r,energy] = solver_QuasiStatic(x,n_tot,idb,bodyForce,bc_set,family,partialAreas,T,c,model,par_omega,ndof,A,damage);
            otherwise
