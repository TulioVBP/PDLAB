...
function [un,r] = solver_QuasiStatic(x,n_tot,b,idb,family,partialAreas,T,ndof,V)
N = length(idb);
...
for n = 1:n_tot
    ...
    %          for the load step.
    epsilon = 10^-4;
    r_vec = getForce(x,un(:,n),T,bn,family,partialAreas,idb,V); % Update to include arbitrary displacement kinematic conditions
...
function f = getForce(x,u,T,b,familyMat,partialAreas,idb,V)
N = length(x);
f = zeros(size(u));
for ii = 1:N
